...
import lipnet.lipnet_layers as layers
...
class LipNet(object):
    ...
    def __init__(self, frame_count: int, image_channels: int, image_height: int, image_width: int, max_string: int,
        ...
        # self.loss_out = layers.create_ctc_layer('ctc', self.y_pred, self.input_labels, self.input_length,
        #                                         self.label_length)

        self.model = keras.models.Model(
            inputs=self.input_layer, outputs=self.y_pred)  # outputs=self.loss_out)
            # inputs=[self.input_layer, self.input_labels, self.input_length, self.label_length], outputs=self.y_pred)  # outputs=self.loss_out)

    def compile_model(self, optimizer=None):
        if optimizer is None:
            optimizer = keras.optimizers.Adam(lr=ADAM_LEARN_RATE, beta_1=ADAM_F_MOMENTUM, beta_2=ADAM_S_MOMENTUM,
                                              epsilon=ADAM_STABILITY)

        # self.model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer=optimizer)
        self.model.compile(loss={'softmax': lambda y_true, y_pred: y_pred}, optimizer=optimizer)
        return self
    ...
    def capture_softmax_output(self):
        return keras.backend.function([self.input_layer, keras.backend.learning_phase()], [self.y_pred])
