...
def main(config):
    ...
    training_data_generator = load_dataset_features.DataGeneratorFaceCNN(
    ...
    validation_data_generator = load_dataset_features.DataGeneratorFaceCNN(
        data_folder=config.data_dir,
        test_actors=config.test_actors,
        frame_num=config.num_frames,
        batch_size=config.batch_size,
...
if __name__ == '__main__':
    ...
    training_config_path = Configuration.load_from_yaml_file(
        os.path.join(args.config_dir, "fer_speaking_effect_cnn_train_config.yml")
    )
